...
>(({ className, variant, children, ...props }, ref) => {
  return (
    ...
    >
      {children}
      <div className="absolute bottom-0 left-0 h-1 w-full bg-gray-200 dark:bg-gray-700 overflow-hidden">
        <div 
          className={cn(
            "h-full bg-primary animate-toast-progress",
            variant === "success" && "bg-success",
            variant === "warning" && "bg-warning",
            variant === "destructive" && "bg-destructive",
            variant === "info" && "bg-info"
          )}
          style={{ animationDuration: `${props.duration || 5000}ms` }} // Use duration prop for animation
        />
      </div>
    </ToastPrimitives.Root>
  )
