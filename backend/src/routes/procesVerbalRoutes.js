...
router.post('/', async (req, res) => {
  ...
  try {
    let generatedCode = null; // PVs might not have a structured code by default, or we can generate one
    ...
    if (company_code && scope_code && department_code && language_code) {
      const { generatedCode: newGeneratedCode, sequence_number: newSequenceNumber } = await generateCodeAndSequence(
        ...
        department_code,
        sub_department_code,
        'PV', // Document type code for PV
        null, // correspondence_type_code is null for PVs
        language_code
      );
...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    const codificationFieldsChanged = [
      ...
      'language_code' // 'meetingType' could also be part of code generation if needed
    ...
    if (codificationFieldsChanged) {
      ...
      updates.code = generatedCode; // Add a 'code' field to PV if not already present
